...
# ---------- Helper Functions ----------
def start_download_streamlit(url: str, media: str, quality: str, cookie_name: str = None):
    """
    Streamlit-specific download starter function
    """
    # Convert cookie_name to cookie_file path
    cookie_file_path = None
    if cookie_name and cookie_name != "None":
        if cookie_name == "default":
            cookie_file_path = str(Path.cwd() / "cookies.txt")
        else:
            cookie_file_path = str(Path.cwd() / "cookies" / f"{cookie_name}.txt")
    ...
    # Start download in thread for Streamlit
...
with col_left:
    ...
    if start:
        if not url.strip():
            ...
            # Start download with corrected function call
            session_id = start_download_streamlit(
                url=url.strip(),
                media=media,
                quality=quality,
                ...
                try:
                    with open(prog.filepath, "rb") as f:
                        st.download_button(
                            "Download file", 
                            f, 
                            file_name=Path(prog.filepath).name
                        )
                except Exception:
